    ...
    <motion.div className='flex h-full items-center justify-center p-5'>
      <div className='h-full border-l-2 border-paper-dark'>
        <ol className='flex flex-col h-full whitespace-nowrap'>
          ...
          {
            pages.length < 15 && <motion.li className={`flex text-xs items-center basis-0 grow-[1] select-none transition-all
            ease-out-cubic origin-left hover:underline`}
              onClick={newPage}
              whileHover={{ scaleX: 1.1, scaleY: 1.1 }}
              whileTap={{ scaleX: 1, scaleY: 1 }}>
              <div className='rounded-full border-double w-4 h-4 bg-white border-paper-dark border-4
                mr-1 -ml-[9px] after:content-none after:absolute after:bg-white'
              ></div>
